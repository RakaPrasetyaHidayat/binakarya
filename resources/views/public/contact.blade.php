    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8 lg:gap-10">
        ...
        <div class="lg:col-span-2">
            <div class="space-y-3 mb-6">
                <h2 class="text-2xl sm:text-3xl font-bold text-gray-900 dark:text-white transition-colors">{{ $siteSettings->get('contact_form_title', 'Kirim Pesan') }}</h2>
                <p class="text-sm sm:text-base text-gray-600 dark:text-gray-400 transition-colors leading-relaxed">{{ $siteSettings->get('contact_form_description', 'Tulis pesan Anda dan tim kami akan segera menghubungi Anda.') }}</p>
            </div>

            <div class="bg-white dark:bg-slate-800 rounded-xl shadow-md border border-gray-100 dark:border-slate-700 transition-colors p-6 sm:p-8">
                @if(session('success'))
                    <div class="mb-6 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 text-green-800 dark:text-green-300 px-4 py-3 rounded-lg text-sm font-medium">
                        {{ session('success') }}
                    </div>
                @endif
                    ...
                    <div class="space-y-5">
                        <div>
                            ...
                            <input type="text" name="name" value="{{ old('name') }}"
                                   class="w-full rounded-lg border border-gray-300 dark:border-slate-600 bg-white dark:bg-slate-700 text-gray-900 dark:text-white px-4 py-2.5 text-sm focus:ring-2 focus:ring-primary-500 focus:border-transparent transition @error('name') border-red-400 @enderror"
                                   placeholder="Nama lengkap Anda">
                            @error('name') <p class="text-red-500 text-sm mt-1">{{ $message }}</p> @enderror
                        </div>

                        <div>
                            <label class="block text-sm font-semibold text-gray-900 dark:text-gray-200 mb-2 transition-colors">Email <span class="text-red-500">*</span></label>
                            <input type="email" name="email" value="{{ old('email') }}"
                                   class="w-full rounded-lg border border-gray-300 dark:border-slate-600 bg-white dark:bg-slate-700 text-gray-900 dark:text-white px-4 py-2.5 text-sm focus:ring-2 focus:ring-primary-500 focus:border-transparent transition @error('email') border-red-400 @enderror"
                                   placeholder="email@contoh.com">
                            @error('email') <p class="text-red-500 text-sm mt-1">{{ $message }}</p> @enderror
                        </div>

                        <div>
                            <label class="block text-sm font-semibold text-gray-900 dark:text-gray-200 mb-2 transition-colors">Subjek</label>
                            <input type="text" name="subject" value="{{ old('subject') }}"
                                   class="w-full rounded-lg border border-gray-300 dark:border-slate-600 bg-white dark:bg-slate-700 text-gray-900 dark:text-white px-4 py-2.5 text-sm focus:ring-2 focus:ring-primary-500 focus:border-transparent transition"
                                   placeholder="Apa yang bisa kami bantu?">
                        </div>

                        <div>
                            <label class="block text-sm font-semibold text-gray-900 dark:text-gray-200 mb-2 transition-colors">Pesan <span class="text-red-500">*</span></label>
                            <textarea name="message" rows="5"
                                      class="w-full rounded-lg border border-gray-300 dark:border-slate-600 bg-white dark:bg-slate-700 text-gray-900 dark:text-white px-4 py-2.5 text-sm focus:ring-2 focus:ring-primary-500 focus:border-transparent transition resize-none @error('message') border-red-400 @enderror"
                                      placeholder="Tulis pesan Anda di sini...">{{ old('message') }}</textarea>
                            @error('message') <p class="text-red-500 text-sm mt-1">{{ $message }}</p> @enderror
                        </div>

                        <button type="submit" class="w-full bg-primary-600 hover:bg-primary-700 text-white font-semibold py-3 text-base rounded-lg transition duration-200 shadow-md hover:shadow-lg">
        ...
        <div class="lg:col-span-1">
            {{-- Main Contact Card --}}
            <div class="relative overflow-hidden rounded-xl shadow-md transition-colors"
                ...
                {{-- Decorative circles --}}
                <div class="absolute -top-4 -right-4 w-16 h-16 rounded-full opacity-10 bg-white"></div>
                <div class="absolute -bottom-3 -left-3 w-14 h-14 rounded-full opacity-10 bg-white"></div>

                <div class="relative z-10 p-6 sm:p-8">
                    <h3 class="text-lg font-bold text-white mb-6">Informasi Kontak</h3>
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="flex gap-3 items-start">
                            <div class="w-10 h-10 rounded-lg bg-white/15 backdrop-blur-sm flex items-center justify-center flex-shrink-0">
                                <svg class="w-5 h-5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z"/></svg>
                            </div>
                            <div>
                                <p class="text-white/70 text-xs font-semibold mb-1">Alamat</p>
                        ...
                        <div class="flex gap-3 items-start">
                            ...
                            <div>
                                <p class="text-white/70 text-xs font-semibold mb-1">Email</p>
                                <a href="mailto:{{ $siteSettings->get('email') }}" class="text-white text-sm hover:text-white/80 transition break-all">{{ $siteSettings->get('email') }}</a>
                            </div>
                        </div>
                        @endif
                        ...
                        <div class="flex gap-3 items-start">
                            <div class="w-10 h-10 rounded-lg bg-green-500/30 backdrop-blur-sm flex items-center justify-center flex-shrink-0">
                            ...
                            <div>
                                <p class="text-white/70 text-xs font-semibold mb-1">WhatsApp</p>
                                <a href="https://wa.me/{{ $siteSettings->get('wa_number') }}" target="_blank" rel="noopener" class="text-white text-sm hover:text-green-300 transition">
                                    {{ $siteSettings->get('wa_number', '0895611314372') }}
                                </a>
                            </div>
                        </div>
                        @endif
                    ...
                    <div class="mt-6 pt-6 border-t border-white/20">
                        <p class="text-white/70 text-xs font-semibold mb-3">Ikuti Kami</p>
                        <div class="flex gap-2 flex-wrap">
                            @if($siteSettings->get('facebook'))
                            <a href="{{ $siteSettings->get('facebook') }}" target="_blank" rel="noopener"
                               class="w-7 h-7 rounded-md bg-white/15 hover:bg-blue-500 flex items-center justify-center transition-all duration-300 hover:scale-110"
                               title="Facebook">
                            ...
                            <a href="{{ $siteSettings->get('instagram') }}" target="_blank" rel="noopener"
                               class="w-7 h-7 rounded-md bg-white/15 hover:bg-gradient-to-br hover:from-purple-600 hover:via-pink-500 hover:to-orange-400 flex items-center justify-center transition-all duration-300 hover:scale-110"
                               title="Instagram">
                            ...
                            <a href="{{ $siteSettings->get('tiktok') }}" target="_blank" rel="noopener"
                               class="w-7 h-7 rounded-md bg-white/15 hover:bg-gray-900 flex items-center justify-center transition-all duration-300 hover:scale-110"
                               title="TikTok">
                            ...
                            <a href="{{ $siteSettings->get('youtube') }}" target="_blank" rel="noopener"
                               class="w-7 h-7 rounded-md bg-white/15 hover:bg-red-600 flex items-center justify-center transition-all duration-300 hover:scale-110"
                               title="YouTube">
                    ...
                    <div class="mt-6 pt-6 border-t border-white/20">
                        <a href="https://wa.me/{{ $siteSettings->get('wa_number') }}" target="_blank" rel="noopener"
                           class="w-full flex items-center justify-center gap-2 bg-green-500 hover:bg-green-600 text-white font-semibold py-1.5 px-3 rounded-md transition-all duration-300 hover:shadow-lg hover:shadow-green-500/30 active:scale-95 text-xs">
